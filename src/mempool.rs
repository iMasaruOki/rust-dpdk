...
pub unsafe fn ops_enqueue_bulk(mp: *mut ffi::rte_mempool,
                               obj_table: &[*mut c_void],
                               len: usize) {
    let ops = get_ops((*mp).ops_index as usize);
    (*ops).enqueue.unwrap()(mp,
                            obj_table.as_ptr() as *const *const c_void,
                            len as u32);
}

pub unsafe fn generic_put(mp: *mut ffi::rte_mempool,
                          obj_table: &[*mut c_void],
    ...
    if cache.is_null() || obj_table.len() > ffi::RTE_MEMPOOL_CACHE_MAX_SIZE as usize {
        ...
        libc::memcpy((*cache).objs.as_mut_ptr().offset((*cache).len as isize) as *mut libc::c_void,
               obj_table.as_ptr() as *mut libc::c_void,
               ::std::mem::size_of::<*mut u32>() * obj_table.len());
        (*cache).len += obj_table.len() as u32;
        if (*cache).len >= (*cache).flushthresh {
            ops_enqueue_bulk(mp, &[(*cache).objs[(*cache).size as usize]],
...
pub unsafe fn default_cache(mp: *mut ffi::rte_mempool, lcore_id: u32)
