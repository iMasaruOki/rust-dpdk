...
impl Port {
    ...
    pub unsafe fn info(&self, info: &mut ffi::rte_eth_dev_info) {
        ffi::rte_eth_dev_info_get(self.port_id,
                                  info as *mut ffi::rte_eth_dev_info);
    }

    pub unsafe fn configure(&self, nb_rxd: u16, nb_txd: u16,
                            port_conf: &ffi::rte_eth_conf) -> i32 {
        ffi::rte_eth_dev_configure(self.port_id, nb_rxd, nb_txd,
    ...
    pub unsafe fn adjust_rx_desc(&self, nb_rxd: u16) -> u16 {
        let mut nbr = nb_rxd;
        let mut nbt = nb_rxd;
        let rv = ffi::rte_eth_dev_adjust_nb_rx_tx_desc(self.port_id,
                                                       &mut nbr, &mut nbt);
        assert_eq!(rv, 0);
        nbr
    }
    ...
    pub unsafe fn rx_queue_setup(&self, queue_id: u16, nb_rxd: u16,
                                 socket_id: u32, rxconf: *mut ffi::rte_eth_rxconf,
                                 pool: *mut ffi::rte_mempool)
                                 -> i32 {
        ffi::rte_eth_rx_queue_setup(self.port_id, queue_id, nb_rxd, socket_id,
                                    rxconf, pool)
    }

    pub unsafe fn tx_queue_setup(&self, queue_id: u16, nb_txd: u16,
                                 socket_id: u32, txconf: *mut ffi::rte_eth_txconf)
                                 -> i32 {
        ffi::rte_eth_tx_queue_setup(self.port_id, queue_id, nb_txd, socket_id, txconf)
    }

    pub unsafe fn rx_burst(&self, queue_id: u16,
    ...
    pub unsafe fn tx_burst(&self, queue_id: u16,
...
impl tx_buffer {
    ...
    pub unsafe fn flush(&mut self, port: Port, queue_id: u16) -> u16 {
        ...
        let sent = port.tx_burst(queue_id,
